...
def get_dataset(mode='train', split='balanced'):
    ...
    dataset = EMNIST(root='./data', split=split, train=True if mode == 'train' else False, download=True,
                     transform=transform)
...
def devide_train_data(data, n_sample, SRC_CLASSES, NUM_USERS, min_sample, alpha=0.5, sampling_ratio=0.5):
    ...
    # arg.min_sample每个用户最小取样数，SRC_CLASSES是EMNIST该类别（letters，byclass）的label数，
    # 相乘得到的是每个用户需要从dataset里面获取的样本数量
    ...
    while min_size < min_sample:
        ...
        for l in SRC_CLASSES:
            ...
            proportions = np.array(
                [p * (n_per_user < max_samples_per_user) for p, n_per_user in zip(proportions, samples_per_user)])
            proportions = proportions / proportions.sum()
            proportions = (np.cumsum(proportions) * len(idx_l)).astype(int)[:-1]
            # 长度user-1
            # cumsum表示累加和，默认是np。cumsum(axis=1),第二列=本列+上一列，第三列=本列+上两列，加完之后行排列成为行向量
...
def main():
    ...
    parser.add_argument("--split", type=str, default='letters',
                        choices=["letters", "balanced", "bymerge", "byclass", "digits"])
    ...
    def process_user_data(mode, data, n_sample, SRC_CLASSES, Labels=None, unknown_test=0):
        ...
        if mode == 'train':
            for u in range(NUM_USERS):
                ...
                print(train_info)
                print("{} Labels/ {} Number of training samples for user [{}]:".format(len(Labels[u]), n_samples_for_u,
                                                                                       u))
